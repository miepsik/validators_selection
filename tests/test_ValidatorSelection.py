...
class ValidatorsSelectionTest(unittest.TestCase):
    ...
    def test_fileuploadNoFile(self):
        result = self.app.post('fileuploadPolkadot', data={'validator':(open('validators.csv', 'rb'), 'validators.csv'), 'depth':2})
        self.assertEqual(result.data, b"Validators file not provided")
        
    def test_fileuploadWrongFormat(self):
        result = self.app.post('fileuploadPolkadot', data={'validators':(open('validatorsWrong.csv', 'rb'), 'validatorsWrong.csv'), 'depth':2})
        self.assertEqual(result.data, b"Invalid file structure, expected 7 columns, got 6")

    def test_fileupload(self):
        result = self.app.post('fileuploadPolkadot', data={'validators':(open('validators.csv', 'rb'), 'validators.csv'), 'depth':2})
    ...
    def test_query(self):
        ...
        self.assertEqual(result.data, b"Done")
        start = time.time()
        result = self.app.post('nextPolkadot', data=json.dumps({'history':[]}), content_type='application/json')
        response = eval(result.data)
        quality0 = response['quality']
        history = response['validatorA']['history']
        result = self.app.post('nextPolkadot', data=json.dumps({'history':history}), content_type='application/json')
        response = eval(result.data)
    ...
    def test_query2(self):
        result = self.app.post('fileuploadPolkadot', data={'validators':(open('validatorsEval.csv', 'rb'), 'validatorsEval.csv'), 'depth':2})
        self.assertEqual(result.data, b"Done")
        model = {'clusterSize': [[1.0, 5.0, 9.0, 13.0], [0.0, 0.04, 0.07, 0.06]], 'commission': [[0.5, 1.0, 4.0, 7.0, 10.0], [0.25, 0.18, 0.12, 0.06, 0.0]], 'eraPoints': [[780, 926, 1073, 1220], [0.0, 0.05, 0.12, 0.2]], 'selfStake': [[1.0, 201480, 402960, 604440.2], [0.0, 0.08, 0.15, 0.22]], 'totalStake': [[1942391.7, 2728076, 3513760, 4299445.4], [0.22, 0.14, 0.07, 0.0]], 'voters': [[279.0, 1042, 1805, 2569.0], [0.0, 0.02, 0.01, 0.02]]}
        response = self.app.post('rankingPolkadot', data=json.dumps({'model':model}), content_type='application/json')
